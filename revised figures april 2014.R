...
par(cex=.7)
plot(pca_results$x,xlim=c(-.25,.25),ylim=c(-.25,.25),las=1,xlab='Principal Component Axis 1',ylab='Principal Component Axis 2',axes=F)
...
par(mar=c(4.1, 4.1, 0.1, 0))
par(cex=.7)
plot(pca_results$rotation,xlim=c(-.75,.75),ylim=c(-.75,.75),type='n',xlab='Principal Component Axis 1',ylab='Principal Component Axis 2',axes=F)
abline(h=0,lty=2,col='darkgrey')
abline(v=0,lty=2,col='darkgrey')
arrows(rep(0,9),rep(0,9),pca_results$rotation[,1],pca_results$rotation[,2],length=.1,lwd=2)
text(pca_results$rotation[,1]+c(.1,.15,.1,.15,-.1,-.1,.15,.11,.11),pca_results$rotation[,2]+c(.05,.05,.05,-.05,.05,-.05,-.05,.05,.05),pause_names,cex=1.2)
axis(1,at=c(-.75,0,.75))
axis(2,las=1,at=c(-.75,0,.75),labels=c(-.75,0,.75))
...
axis(1,at=c(-3,0,3))
axis(2,las=1,at=c(-5.5,0,5.5),labels=c(-5.5,0,5.5))
dev.off()

tiff("~/Documents/shakespeare-chronordination/figure4.tiff",width=6,height=6,units='in',res=720)
par(mfrow=c(1,1))
xlimit=-.75
ylimit=.7
...
abline(h=0,lty=2,col='darkgrey')
abline(v=0,lty=2,col='darkgrey')
for (i in sh_ind) {polygon(multi_out[4][[1]][[i]],col=alpha('darkgrey',.2),border='darkgrey')}
centers = t(t(multi_out[[3]]) %*% t(as.matrix(multi[,3:11])))/apply(multi[,3:11],1,sum)
text(centers[multi_indicies,1]+c(.25,.3,.1,-.2,-.35),centers[multi_indicies,2]+c(.225,-.125,.3,.25,.15),labels=multi$Title[multi_indicies],cex=.85,col='black',font=3)
segments(centers[multi_indicies,1]+c(.065,.05,0,-.05,-.09),centers[multi_indicies,2]+c(0,0,.085,.05,0),centers[multi_indicies,1]+c(.25,.17,.1,-.15,-.275),centers[multi_indicies,2]+c(.17,-.1,.25,.2,0.1))
for (i in multi_indicies) {polygon(multi_out[4][[1]][[i]],col=alpha('black',0.4))}
axis(1,at=c(-.7,0,.7),labels=c(.7,0,-.7))
axis(2,las=1,at=c(-.7,0,.7))
dev.off()

tiff('~/Documents/shakespeare-chronordination/figure5.tiff',width=8,height=6,units='in',res=720)
par(mfrow=c(2,3))
for (i in 1:length(authors)){
  plot(x=0,y=0,type='n',xlim=c(-xlimit,xlimit),ylim=c(-ylimit,ylimit),xlab='CA Axis 1',ylab='CA Axis 2',las=1,asp=1,axes=F)
  abline(h=0,lty=2,col='darkgrey')
  abline(v=0,lty=2,col='darkgrey')
  for (j in sh_ind) {polygon(multi_out[4][[1]][[j]],col=alpha('darkgrey',.2),border='darkgrey')}
  for (k in authors[[i]]) {polygon(multi_out[4][[1]][[k]],col=alpha('black',.4))}
  axis(1,at=c(-.7,0,.7),labels=c(.7,0,-.7))
  axis(2,las=1,at=c(-.7,0,.7))
...
setEPS()
postscript("~/Documents/shakespeare-chronordination/figure6.eps",width=6.5,height=3.5)
par(mfrow=c(1,1))
par(mar=c(4.1, 4.1, 0.1, 0))
par(cex=.7)
plot(x_vals,y_vals,axes=F,xlab='Year of composition',ylab='CA Axis 1 Score',xlim=c(1590,1614),pch=19,ylim=c(-2,2))
axis(1,at = seq(1590,1614,4))
axis(2,las=1,at=seq(-2,2,.5))
abline(mod,lwd=2,lty=2)
text(x_vals+.5,rep(-1.8,3),anchors,cex=.9,font=4,adj=c(0,0))
text(x_vals+.5,rep(-2,3),paste(x_vals),adj=c(0,0),cex=.9)
arrows(x_vals,y_vals,rep(1589.1,3),y_vals,lty=1,length=.1,lwd=2)
arrows(x_vals,rep(-2.5,3),x_vals,y_vals,lty=1,length=.1,lwd=2)
arrows(matlab$V1[matlab$Title=='Tempest'],-2.5,matlab$V1[matlab$Title=='Tempest'],mod$coefficients[1]+mod$coefficients[2]*matlab$V1[matlab$Title=='Tempest'],length=.1,lwd=2)
arrows(matlab$V1[matlab$Title=='Tempest'],mod$coefficients[1]+mod$coefficients[2]*matlab$V1[matlab$Title=='Tempest'],1589.1,mod$coefficients[1]+mod$coefficients[2]*matlab$V1[matlab$Title=='Tempest'], length=.1,lwd=2)
text(matlab$V1[matlab$Title=='Tempest']+.5,-1.8,'Tempest',font=4,adj=c(0,0),cex=.9)
text(matlab$V1[matlab$Title=='Tempest']+.5,-2,paste('>',matlab$V1[matlab$Title=='Tempest'],sep=' '),adj=c(0,0),cex=.9)
segments(0,ca_results$rowcoord[data$Title=='Macbeth',1],matlab$V1[matlab$Title=='Macbeth'],mod$coefficients[1]+mod$coefficients[2]*matlab$V1[matlab$Title=='Macbeth'],lwd=2,col='darkgrey')
arrows(matlab$V1[matlab$Title=='Macbeth'],mod$coefficients[1]+mod$coefficients[2]*matlab$V1[matlab$Title=='Macbeth'],matlab$V1[matlab$Title=='Macbeth'],-2.15,length=.1,lwd=2,lty=1,col='darkgrey')
text(matlab$V1[matlab$Title=='Macbeth']-.25,-1,'Macbeth\n  ',cex=.9,adj=c(1,0),font=4,col='darkgrey')
text(matlab$V1[matlab$Title=='Macbeth']-.25,-1.2,paste('  \nCA score = ',round(mod$coefficients[1]+mod$coefficients[2]*matlab$V1[matlab$Title=='Macbeth'],3),'\nPredicted year = ',matlab$V1[matlab$Title=='Macbeth'],sep=''),cex=.9,adj=c(1,0),col='darkgrey',font=2)
dev.off()
...
setEPS()
postscript("~/Documents/shakespeare-chronordination/figure9.eps",width=6.5,height=8)
par(mfrow=c(1,1))
par(mar=c(4.1, 4.1, 0.1, 0))
par(cex=.7)
plot(temp$V1,1:42,xlim=c(1590,1620),pch=19,xlab='',ylab='Play order',axes=F,ylim=c(42,1),type='n')
...
axis(1,at=seq(1590,1615,5))
points(shifted,1:42,col='black',pch=21,bg='white')
points(mean_boots,1:42,pch=19,col='black')
mtext(text='Time',side=1,line=3,cex=.7,at = 1602.5)
dev.off()


setEPS()
postscript("~/Documents/shakespeare-chronordination/figure10.eps",width=6.5,height=8)
par(mfrow=c(1,1))
par(mar=c(4.1, 4.1, 0.1, 0))
par(cex=.7)
plot(step3$oxford,1:42,xlim=c(1590,1620),pch=19,xlab='',ylab='Play order',axes=F,ylim=c(42,1),type='n')
for (i in 1:length(close)) {
  polygon(c(close[i],open[i],open[i],close[i]),c(0,0,42,42),col='lightgrey',border=NA)
}
